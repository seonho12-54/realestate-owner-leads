  ...
  useEffect(() => {
    ...
    listActiveOffices()
      ...
      .catch((loadError) => {
        ...
        setOffices([]);
        setError(loadError instanceof Error ? loadError.message : "매물 등록에 필요한 중개사무소 목록을 불러오지 못했습니다.");
      })
      .finally(() => {
        if (isMounted) {
          setIsLoading(false);
    ...
    return (
      <div className="page-stack">
        <section className="page-panel">
          ...
          <p className="page-copy compact-copy">사진 업로드와 등록 폼 연결 상태를 확인한 뒤 바로 이어서 입력할 수 있게 불러오고 있습니다.</p>
        </section>
      </div>
    );
  }
  ...
  if (!session.authenticated || (session.kind !== "user" && session.kind !== "admin")) {
    return (
      <div className="page-stack">
        <section className="page-panel">
          <span className="eyebrow">매물 등록</span>
          <h1 className="page-title page-title-medium">로그인 후 바로 매물을 등록할 수 있습니다.</h1>
          <p className="page-copy compact-copy">사진 업로드까지 한 번에 처리되도록 회원 로그인 상태에서 등록 페이지를 열어 주세요.</p>
  ...
  if (session.kind !== "admin" && !session.region.locked) {
    return (
      <div className="page-stack">
        <section className="page-panel">
          <span className="eyebrow">매물 등록</span>
          <h1 className="page-title page-title-medium">지역 인증 후 매물 등록을 진행해 주세요.</h1>
          <p className="page-copy compact-copy">등록 매물은 인증된 생활권 기준으로 연결되기 때문에 먼저 현재 동네 인증이 필요합니다.</p>
          <div className="button-row" style={{ marginTop: 8 }}>
  ...
  return (
    <div className="page-stack">
      <section className="hero-card">
        <div>
          <span className="eyebrow">매물 등록</span>
          <h1 className="page-title page-title-medium">지금 보고 있는 스타일 그대로 매물을 바로 접수해 보세요.</h1>
          <p className="page-copy">
            주소 검색, 지도 미리보기, 사진 업로드, 등록 완료까지 한 번에 이어집니다. 등록된 매물은 관리자 확인 후 공개 목록에 반영됩니다.
          </p>
        </div>
        <div className="hero-region-card">
          <span className="eyebrow" style={{ fontSize: "0.7rem" }}>
            {session.kind === "admin" ? "관리자 등록" : "등록 가능 지역"}
          </span>
          <strong>{session.kind === "admin" ? "관리자 권한으로 등록 중" : session.region.region?.name ?? "인증된 지역"}</strong>
          <p>
            {session.kind === "admin"
              ? "관리자 계정은 등록과 사진 업로드를 바로 진행할 수 있습니다."
              : "일반회원 등록은 현재 인증된 생활권 기준으로 접수되고, 사진도 함께 업로드됩니다."}
          </p>
        </div>
      </section>
      ...
      {error ? (
        <section className="page-panel">
          ...
          <h2 className="section-title">등록 화면을 완전히 열지 못했습니다.</h2>
      ...
      {offices.length === 0 ? (
        <section className="page-panel">
          ...
          <p className="page-copy compact-copy">관리자에서 사무소 정보를 먼저 확인한 뒤 다시 시도해 주세요.</p>
        </section>
      ) : (
        <SellLeadForm
          offices={offices}
